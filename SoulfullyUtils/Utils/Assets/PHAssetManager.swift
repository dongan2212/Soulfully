...
public struct PHAssetManager {
  ...
  static let imageRequestOption: PHImageRequestOptions = {
  ...
  static let requestOption: PHImageRequestOptions = {
  ...
  static let optionNonSync: PHImageRequestOptions = {
    let option = PHImageRequestOptions()
    option.isNetworkAccessAllowed = true
    return option
  }()
  ...
  static func authorizeForAccess(_ allowed: @escaping Action, _ unallowed: @escaping Action) {
  ...
  static func fetchScreenShots(limit: Int) -> [PHAsset] {
    let options = PHFetchOptions()
    ...
    let fetchResult: PHFetchResult = PHAsset.fetchAssets(with: PHAssetMediaType.image, options: options)
    return toArray(fetchResult)
  }
  ...
  static func retrieveAlbums<T>(filter: @escaping (T, String) -> Bool, create: @escaping (String, Int, UIImage, PHFetchResult<PHAsset>) -> T) -> (PHFetchResult<PHAssetCollection>, [T]) {
    var albums = [T]()
  ...
  static func retrieveImage(asset: PHAsset, size: CGFloat = 70, sync: @escaping (UIImage?) -> Void) {
  ...
  static func retrieveImage(from asset: PHAsset, option: PHImageRequestOptions = PHAssetManager.optionNonSync, sync: @escaping (UIImage) -> Void) {
    DispatchQueue.global(qos: .utility).async {
