...
public extension UIView {
    ...
    func fadeAnimation(isShow: Bool = true, spring: Bool = false, duration: TimeInterval = 0.25, delay: TimeInterval = 0, completion: (() -> Void)? = nil) {
    ...
    func layoutAnimation(duration: TimeInterval, action: @escaping Action) {
        action()
    ...
    ///   - duration: animation duration
    func rotateAnimation(rad: CGFloat = .pi * 2, duration: TimeInterval = 0.5) {
    ...
    ///   - delay: delay time
    ///   - scale: scale value
    func locationAnimation(duration: TimeInterval = 2, delay: TimeInterval = 0, scale: CGFloat = 0 ) {
    ...
    ///   - duration: animation duration
    ///   - delay: delay time
    func moveAnimation(duration: TimeInterval, delay: TimeInterval) {
    ...
    ///   - duration: animation duration
    ///   - delay: delay time
    func rotateAnimationMask(duration: TimeInterval, delay: TimeInterval) {
    ...
    /// - Parameters:
    ///   - duration: animation duration
    func shake(duration: CFTimeInterval = 0.01,
    ...
    ///   - duration: animation duration
    ///   - delay: delay time
    func hideAnimation(duration: TimeInterval, delay: TimeInterval, completion: Action? = nil) {
        UIView.animate(withDuration: duration, delay: delay, options: UIView.AnimationOptions.curveEaseIn, animations: {
            self.alpha = 0
        }, completion: { _ in
            completion?()
    ...
    func zoomAnimation(duration: TimeInterval,
                       delay: TimeInterval,
                       scaleX: CGFloat,
                       scaleY: CGFloat,
    ...
    func slideInAnimation(duration: Double = 1, subType: CATransitionSubtype = CATransitionSubtype.fromBottom) {
        alpha = 1
